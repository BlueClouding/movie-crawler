...
from common.db.entity.movie_info import MovieInfo
...
async def get_movie_detail_crawler_service() -> MovieDetailCrawlerService:
    ...
    async with async_session() as session:
        ...
        movie_info_repo = MovieInfoRepository(session)
        download_url_repo = DownloadUrlRepository(session)
        logger.info(
            f"Creating MovieDetailCrawlerService with MovieInfoRepository: {type(movie_info_repo).__name__}"
        )
        logger.info(f"DownloadUrlRepository: {type(download_url_repo).__name__}")
        yield MovieDetailCrawlerService(
            progress_service, movie_info_repo, movie_repo, download_url_repo
        )
...
async def start_crawler(
    ...
    if task_status["is_running"]:
        return TaskResponse(
            success=False, message="爬虫任务已在运行中", status=task_status
        )
    ...
    # 添加后台任务
    background_tasks.add_task(run_crawler_task, batch_size, headless, movie_service)
...
async def crawl_movie_details(
    ...
    movie_detail_crawler_service: MovieDetailCrawlerService = Depends(
        get_movie_detail_crawler_service
    ),
):
    """
    爬取电影详情
    ...
    if task_status["is_running"]:
        return TaskResponse(
            success=False, message="已有爬虫任务在运行中", status=task_status
        )
    ...
    async with async_session() as session:
        try:
            ...
            AND m.code NOT LIKE '%Uncensored-Leaked%'
            ORDER BY m.id ASC
            LIMIT :limit
            """
            )
            ...
            movies_to_crawl: List[MovieInfo] = result.all()

            if not movies_to_crawl:
                return TaskResponse(
                    success=False, message="没有找到需要爬取详情的电影", task_id=None
                )

            movie_codes = [movie.code for movie in movies_to_crawl]
        
            logger.info(
                f"找到 {len(movie_codes)} 部需要爬取详情的电影: {', '.join(movie_codes[:5])}..."
            )
...
async def run_movie_detail_crawler_task(
    ...
    try:
        ...
        results = await movie_detail_crawler_service.batch_crawl_movie_details(
            movie_codes=movie_codes, language=language, headless=headless
        )
        ...
        logger.info(
            f"电影详情爬虫任务完成，成功: {success_count}，失败: {failed_count}"
        )
...
async def stop_crawler():
    ...
    if not task_status["is_running"]:
        return TaskResponse(
            success=False, message="没有正在运行的爬虫任务", status=task_status
        )
...
async def run_crawler_task(
    batch_size: int, headless: bool, movie_service: MovieService
):
    """
    运行爬虫任务
        ...
        for i, movie in enumerate(gaps):
            ...
            try:
                ...
                async with async_session() as session:
                    movie_service = MovieService(session)
                    if success:
                        await movie_service.update_movie_status(
                            movie_id, CrawlerStatus.COMPLETED
                        )
                        task_status["success_count"] += 1
                    else:
                        await movie_service.update_movie_status(
                            movie_id, CrawlerStatus.FAILED
                        )
                        task_status["failed_count"] += 1
